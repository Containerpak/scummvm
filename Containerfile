...
RUN apt update && \
    apt install -y --no-install-recommends libasound2t64 libcurl4t64 libfluidsynth3 libflac14 libfreetype6 libjpeg-turbo8 libpng16-16 libsdl2-2.0-0 libsndio7.0 libtheora1 libtheoradec2 libvorbis0a libvorbisfile3 && \
    ldd /usr/bin/scummvm | tee /tmp/scummvm-ldd && \
    ! grep -q 'not found' /tmp/scummvm-ldd && \
    cpak-clean-junk
